...
if win32api.GetLastError() == winerror.ERROR_ALREADY_EXISTS:
    ...
    root_temp = tk.Tk()
    root_temp.withdraw()
    tkinter.messagebox.showerror("Focus Tracker", "App is already running!")
    sys.exit(0)
    ...
    if not music_playing:
        if os.path.exists(MUSIC_FILE):
            try:
                ...
                import tkinter.messagebox
                tkinter.messagebox.showerror("Music Error", str(e))
        else:
            import tkinter.messagebox
            tkinter.messagebox.showwarning("Missing Music File", "Please place 'focus_music.mp3' in the app folder.")
    else:
...
def processing_loop():
    ...
    try:
        agent = FocusAgentInference(model_path="dqn_focus_agent_v2.1.zip", scaler_path="scaler_v2.1.pkl")
        llm = FocusLLMAgent()
        ...
        # Display the error on the GUI
        err_frame = np.zeros((600, 800, 3), dtype=np.uint8)
        y0, dy = 50, 30
        for i, line in enumerate(err_msg.split('\n')):
            cv2.putText(err_frame, line[:80], (10, y0 + i*dy), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        latest_frame_pil = Image.fromarray(cv2.cvtColor(err_frame, cv2.COLOR_BGR2RGB))
        latest_stats = {
            'action': 'ERROR',
            'msg': 'Processing Loop Crashed!',
            'dist_ratio': 0,
            'cf': 0,
            'person': 0
        }
...
def show_hard_nudge_window(msg):
    global nudge_active, latest_stats
    ...
    def on_click(event=None):
        global nudge_active, latest_stats
        if latest_stats:
            latest_stats['action'] = "Do Nothing" # Reset action to prevent instant respawn
            latest_stats['dist_ratio'] = 0.0 # Reset dist ratio
        nudge_active = False
        nudge_win.destroy()
        
    nudge_win.protocol("WM_DELETE_WINDOW", on_click)
        
    tk.Label(nudge_win, text="FOCUS REQUIRED", fg="red", bg="black", font=("Arial", 50, "bold")).pack(pady=(150, 20))
    tk.Label(nudge_win, text="You have been distracted for a while.\nPlease refocus on your work!", fg="white", bg="black", font=("Arial", 25)).pack(expand=True)
...
def update_gui():
    ...
    if latest_stats:
        ...
        status_text = "FOCUSED" if cf < 30 else ("EARLY WARNING" if cf < 90 else "DISTRACTED")
        color = "#00FF00" if cf < 30 else ("#FFA500" if cf < 90 else "#FF0000")
        
        if latest_stats['person'] == 0:
            status_text = "USER MISSING"
            color = "#FF0000"
        
        status_val.config(text=status_text, fg=color)
        rec_val.config(text=act)
        llm_val.config(text=msg)
        
        if act in ["Sound Alert", "Suggest Short Break"] and cf >= 900:
            ...
            if msg != last_notified_msg and msg != "You're doing great! Keep it up.":
                if time.time() - last_toast_time > 15:
                    show_soft_nudge_window(msg)
                    last_notified_msg = msg
                    last_toast_time = time.time()
                    
    root.after(30, update_gui)

def show_soft_nudge_window(msg):
    nudge = tk.Toplevel(root)
    nudge.title("Focus Reminder")
    nudge.geometry("400x100")
    nudge.configure(bg="#FFA500")
    nudge.attributes('-topmost', True)
    
    # Position in bottom right corner
    x = root.winfo_screenwidth() - 420
    y = root.winfo_screenheight() - 150
    nudge.geometry(f"+{x}+{y}")
    
    tk.Label(nudge, text=msg, font=("Segoe UI", 12, "bold"), bg="#FFA500", fg="black", wraplength=380).pack(expand=True)
    
    # Auto close after 5 seconds
    root.after(5000, nudge.destroy)
